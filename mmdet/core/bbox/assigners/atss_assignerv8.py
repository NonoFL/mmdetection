'''
首先确定true_pos，数量为topk，离gtbbox中心最近， img层次的，算出其thr = mean+std, 这些点中，<max(mean-2*std, 0)的为neg
...
class ATSSAssignerv8(BaseAssigner):
    ...
    def assign(self,
        ...
        zero = torch.zeros_like(ensure_pos_mean)
        posa_in_pos = ensure_pos_overlaps >= torch.max(ensure_pos_mean-2*ensure_pos_std, zero)
